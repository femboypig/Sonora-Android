...
class TrackStore(private val context: Context) {
    ...
    fun setCustomArtwork(trackId: String, sourceUri: Uri): Boolean {
        if (trackId.isBlank()) {
            return false
        }
        ...
        val suggestedName = queryDisplayName(sourceUri) ?: "cover.jpg"
        val importedPath = runCatching {
            context.contentResolver.openInputStream(sourceUri)?.use { input ->
                importArtworkBytes(input.readBytes(), suggestedName)
            }
        }.getOrNull()

        val destinationPath = importedPath?.takeIf { it.isNotBlank() } ?: return false
        val previousPath = current[index].artworkPath
        current[index] = current[index].copy(artworkPath = destinationPath)
        saveTracks(current)

        if (!previousPath.isNullOrBlank() && previousPath != destinationPath) {
